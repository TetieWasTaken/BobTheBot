...
module.exports = {
  ...
  raiseUserHierarchyError: (interaction) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ":x: You cannot perform this action on a member with a higher or equal role than you!"
      )
      .setColor(0xff0000);
    return interaction.reply({ embeds: [embed], ephemeral: true });
  },
  raiseBotHierarchyError: (interaction) => {
    const embed = new EmbedBuilder()
      .setTitle(`Hierarchy Error  •  \`/${interaction.commandName}\``)
      .setDescription(
        ":x: I cannot perform this action on a member with a higher or equal role than me!"
      )
      .setColor(0xff0000);
    return interaction.reply({ embeds: [embed], ephemeral: true });
  },
